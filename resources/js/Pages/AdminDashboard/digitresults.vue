                        ...
                        <table class="table table-dark">
                            ...
                            <tbody>
                                ...
                                <tr v-for="result in results" :key="result.id">
                                    <td>
                                        <a v-if="result.dashboard" href="#">
                                            {{ result.dashboard.dashboard }} | {{ result.dashboard.dashboardType }}
                                        </a>
                                        <span v-else>No Dashboard</span>
                                    </td>
                                    <td>{{ result.dashboard ? result.dashboard.draw : '-' }}</td>
                                    <td>{{ result.dashboard ? result.dashboard.date : '-' }}</td>
                                    <td>{{ result.dashboard ? result.dashboard.price * 70 : '-' }}</td>
                                    <td>{{ result.winning_number }}</td>
                                    <td>{{ result.dashboard ? result.dashboard.draw_number : '-' }}</td>
                                </tr>
...
export default {
    ...
    computed: {
        filteredDashboards() {
            ...
            if (!selectedLottery || !selectedLottery.digitDashboards) {
                return [];
            }

            // Convert dashboards object into an array
            const dashboardsArray = Object.values(selectedLottery.digitDashboards);
    ...
    methods: {
        ...
        async addResult() {
            ...
            try {
                const selectedDashboard = this.filteredDashboards.find(d => d.id === this.newResult.dashboard);
                const response = await axios.post('/api/admin/digitresults/store', {
        ...
        validateWinningNumber() {
            ...
            if (this.newResult.winning_number < 0) {
                this.newResult.winning_number = 0;
            } else if (this.newResult.winning_number > 9) {
                this.newResult.winning_number = 9;
            }

            // Last winning number
            if (this.newResult.lwinning_number < 0) {
                this.newResult.lwinning_number = 0;
            } else if (this.newResult.lwinning_number > 9) {
                this.newResult.lwinning_number = 9;
            }
        }
        ,
