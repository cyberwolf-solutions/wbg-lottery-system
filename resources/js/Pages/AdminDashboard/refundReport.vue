    ...
    <div id="app" class="d-flex dark-theme">
        ...
        <div :class="['main-content', { 'sidebar-hidden': !isSidebarVisible }]" class="flex-fill">
            <div class="dashboard-banner">
                <div class="navbar">
                    <h2 class="lottery-name fw-bold text-danger">Refund Report</h2>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(refund, index) in paginatedRefunds" :key="refund.id">
                                ...
                                <td>
                                    <span v-if="refund.wallet && refund.wallet.user">
                                        {{ refund.wallet.user.name }} || ID: {{ refund.wallet.user.id }}
                                    </span>
                                    <span v-else>N/A</span>
                                </td>
                                <td>${{ safeFormatBalance(refund.amount) }}</td>
                                <!-- <td>{{ refund.type }}</td> -->
                                <td>
                                ...
                                <td>{{ formatDate(refund.transaction_date) }}</td>
    ...
    computed: {
        filteredRefunds() {
            ...
            return this.refunds.filter(refund => {
                // Check basic fields
                if (refund.amount.toString().includes(query) ||
                    refund.transaction_date.toLowerCase().includes(query) ||
                    refund.type.toLowerCase().includes(query)) {
                    return true;
                }

                // Check user name if available
                if (refund.wallet && refund.wallet.user &&
                    refund.wallet.user.name.toLowerCase().includes(query)) {
                    return true;
                }

                // Check lottery name if available
                if (refund.lottery && refund.lottery.name.toLowerCase().includes(query)) {
                    return true;
                }

                // Check dashboard info if available
                if (refund.lottery_dashboard &&
                    (refund.lottery_dashboard.dashboard.toLowerCase().includes(query) ||
