    ...
    <div id="app" class="d-flex dark-theme">
        ...
        <div :class="['main-content', { 'sidebar-hidden': !isSidebarVisible }]" class="flex-fill">
            <div class="dashboard-banner">
                ...
                <div class="lottery-table">
                    ...
                    <div v-for="dashboardGroup in paginatedDashboardGroups" :key="dashboardGroup.name"
                        ...
                        <table class="dashboard-table">
                            ...
                            <tbody>
                                <tr v-for="dashboard in getPaginatedItems(dashboardGroup.name, dashboardGroup.items)"
                                    :key="dashboard.id" @click="viewDetails(dashboard)">

                                    <td>{{ dashboard.draw_number }}</td>
                                    <td>{{ formatDate(dashboard.date) }}</td>
                                    <td>
                                        <span
                                            :class="['status-badge', dashboard.status === 'active' ? 'active' : 'closed']">
                                            {{ dashboard.status }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="percentage-bar">
                                            <div class="percentage-fill"
                                                :style="{ width: calculatePercentage(dashboard.id) + '%' }">
                        ...
                        <div class="pagination-controls mt-2">
                            <button class="btn btn-sm btn-primary" @click="groupPrevPage(dashboardGroup.name)"
        ...
        <div v-if="isPickerOpen" class="modal-overlay">
            <div class="modal-content">
                ...
                <h3>Picked Numbers for {{ currentDashboard?.dashboard }} (Draw #{{ currentDashboard?.draw_number }})
                </h3>
...
export default {
    ...
    data() {
        return {
            ...
            numberOptions: Array.from({ length: 10 }, (_, i) => String(i)),
            hoveredNumber: null,
            hoveredUser: null,
            currentPage: 1,
            perPage: 6,
    ...
    methods: {
        ...
        calculatePercentage(dashboardId) {
            const numbers = this.pickedNumbers[dashboardId] || [];
            const totalSlots = 10;  // ✅ now only 0–9
            const pickedCount = numbers.length;
            return Math.round((pickedCount / totalSlots) * 100);
        },
...
.pagination-controls {
    ...
    gap: 20px;
    margin: 30px 0 10px;
    /* Adjusted margin for bottom placement */
}
