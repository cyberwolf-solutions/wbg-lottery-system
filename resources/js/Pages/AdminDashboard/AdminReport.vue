    ...
    <div id="app" class="d-flex dark-theme">
        ...
        <div :class="['main-content', { 'sidebar-hidden': !isSidebarVisible }]" class="flex-fill">
            <div class="dashboard-banner">
                <div class="navbar">
                    <h2 class="lottery-name fw-bold text-danger">Admins Report</h2>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="admin in paginatedAdmins" :key="admin.id">
                                ...
                                <td>{{ admin.name }}</td>
                                <td>{{ admin.email }}</td>
                                <td>{{ admin.roles?.length ? admin.roles[0].name : 'No Role' }}</td>
                                <td>
                                    <span :class="['badge', admin.is_super_admin ? 'bg-success' : 'bg-danger']">
    ...
    methods: {
        ...
        downloadPDF() {
            ...
            const tableData = this.filteredAdmins.map(admin => [
                ...
                admin.name,
                admin.email,
                admin.roles?.length ? admin.roles[0].name : 'No Role',
                admin.is_super_admin ? 'Yes' : 'No',
                this.formatDate(admin.created_at)
            ]);

            // Add table using autoTable
            autoTable(doc, {
                startY: 30,
                head: [['ID', 'Name', 'Email', 'Role', 'Super Admin', 'Join Date']],
                body: tableData,
                theme: 'striped',
            ...
            // Download the PDF
            doc.save(`admins_report_${date.split(',')[0].replace(/\//g, '-')}.pdf`);
        }
    }
};
</script>
