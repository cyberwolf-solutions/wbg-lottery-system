...
const totalPrice = computed(() => {
    const dashboardPrice = selectedLotteryDetails.value[0]?.price || 0;
    const pickedPrice = pickedNumbers.value.length * parseFloat(dashboardPrice);
    return pickedPrice.toFixed(2);
});
...
const storedNumbers = ref([...Array(10).keys()].map(n => n.toString()));
...
function pickNumberConfirmed() {
    axios.post('/api/digit-pick-number', {
        number: numberToPick.value,
        lottery_dashboard_id: dashboardIdToPick.value,
        lottery_id: props.lotterie.id,
    })
        .then(response => {
            if (response.data?.number !== undefined && response.data?.number !== null) {
                pickedNumbers.value.push({
...
function isNumberPicked(number, dashboardId) {
    return props.pickedNumbers.some(picked =>
        picked.picked_number.toString() === number.toString() && picked.lottery_dashboard_id === dashboardId
    );
}

const showModal = ref(true);
const selectedLottery = ref(props.lotterie?.name || '');
...
function startCountdown() {
    selectedLotteryDetails.value.forEach(ticket => {
        ...
        targetDate.setUTCHours(14, 30, 0, 0);
...
async function checkout() {
    try {
        ...
        const response = await axios.post('/api/digitcheckout', {
            numbers: numbersData,
            lottery_id: props.lotterie.id,
            dashboard_id: dashboardId,
            total_price: totalPrice.value,
...
function deletePickedNumbers() {
    ...
    axios.post('/api/delete-digit-picked-numbers')
        .then(response => {
        ...
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Price {{ selectedLottery ? selectedLottery : 'Lotteries' }} || {{ props.lotterie.name }} || {{
                    selectedLotteryDetails[0]?.dashboardType }}
            </h2>
        </template>
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    ...
                    <div
                        ...
                        <div class="mt-4 sm:mt-0 text-left">
                            ...
                            <div class="allocated-numbers mt-4 sm:mt-0 text-left mx-3">
                                <span>Available balance: {{ wallet.available_balance }}</span>
                                <p class="text-sm text-gray-500">
                                    <b>Note</b> - You have <b>{{ formattedCountdown }}</b> minutes to checkout.<br> If
                                    not
                                    completed, the system will
                                    <br>auto-confirm your selection and <br>proceed with booking
                                </p>
                            </div>
                        </div>
                    </div>
