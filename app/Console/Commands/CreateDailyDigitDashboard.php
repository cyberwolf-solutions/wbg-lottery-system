...
use App\Notifications\NewDigitDashboardCreatedNotification;

class CreateDailyDigitDashboard extends Command
{
    protected $signature = 'generate:digitdashboard';
    ...
    public function handle()
    {
        try {
            ...
            if ($now->format('H:i') == '00:00') {
                ...
                $lotteryDashboardCombinations = DigitLotteryDashboard::select('lottery_id', 'dashboard', 'dashboardType')
                    ->distinct()
                    ->get();
                ...
                foreach ($lotteryDashboardCombinations as $combination) {
                    ...
                    $lastDashboard = DigitLotteryDashboard::where('lottery_id', $lotteryId)
                    ...
                    $existingDashboard = DigitLotteryDashboard::where('lottery_id', $lotteryId)
                        ->where('dashboard', $dashboardField)
                        ->where('dashboardType', $dashboardType)
                ...
                if (!empty($createdDashboards)) {
                    ...
                    foreach ($users as $user) {
                        $user->notify(new NewDigitDashboardCreatedNotification($createdDashboards));
                    }
