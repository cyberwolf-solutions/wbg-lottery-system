...
class DepositStatusNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        $mailMessage = (new MailMessage)
            ->subject('Deposit Request ' . ucfirst($this->status))
            ->line("Your deposit request of {$this->amount} has been {$this->status}.");
    ...
    public function toArray($notifiable)
    {
        $notificationData = [
            'amount' => $this->amount,
            'status' => $this->status,
            'message' => "Your deposit request of {$this->amount} has been {$this->status}.",
        ];
