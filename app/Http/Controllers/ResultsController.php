...
class ResultsController extends Controller
{
    ...
    public function digitindex()
    {
        $lotteries = Lotteries::with(['digitDashboards' => function ($query) {
            $query->where('date', '<', now()->format('Y-m-d'))
                ->where(function ($q) {
                    $q->where('status', 'deactive')
                        ->orWhere('status', 'closed');
        ...
        Log::info('Lotteries with Dashboards', ['lotteries' => $lotteries->toArray()]);

        $results = DigitResult::with('lottery', 'dashboard')
            ->orderBy('lottery_id')
            ->get()
            ->groupBy('lottery_id');
        ...
        foreach ($lotteries as $lottery) {
            ...
            foreach ($lottery->digitDashboards as $key => $dashboard) {
                // Create a unique key with draw_number + dashboardType + dashboard
                $uniqueKey = $dashboard->draw_number . '-' . $dashboard->dashboardType . '-' . $dashboard->dashboard;

                if (in_array($uniqueKey, $seenCombinations)) {
                    // Remove duplicate dashboards (same draw_number + dashboardType + dashboard)
                    unset($lottery->digitDashboards[$key]);
                    continue;
                } else {
                    // Mark this combination as seen
                    $seenCombinations[] = $uniqueKey;
            ...
            // Re-index the array after unsetting elements
            $lottery->digitDashboards = array_values($lottery->digitDashboards->toArray());
        }

        return Inertia::render('AdminDashboard/digitresults', [
            'results' => $results,
            'lotteries' => $lotteries
        ]);
    }
    ...
    public function digitstore(Request $request)
    {
        try {
            ...
            // Fetch the two dashboards (FirstDigit and LastDigit) for the given draw number
            $dashboards = DigitLotteryDashboard::where('draw_number', $validatedData['draw_number'])
                ->where('lottery_id', $validatedData['lottery_id'])
                ->whereIn('dashboardType', ['First Digits', 'Last Digits'])
                ->where('dashboard', $validatedData['dashboard_name'])
                ->where(function ($q) {
            ...
            // Add result to all matching First Digits dashboards
            foreach ($firstDigitDashboards as $fdDashboard) {
                $this->addDigitResultToDashboard($validatedData, $fdDashboard, $validatedData['winning_number']);
            }

            // Add result to all matching Last Digits dashboards
            foreach ($lastDigitDashboards as $ldDashboard) {
                $this->addDigitResultToDashboard($validatedData, $ldDashboard, $validatedData['lwinning_number']);
            }
    ...
    private function addDigitResultToDashboard($validatedData, $dashboard, $winningNumber)
    {
        // Check if the result already exists for this dashboard
        $existingResult = DigitResult::where('lottery_id', $validatedData['lottery_id'])
            ->where('digit_dashboard_id', $dashboard->id)
            ->where('winning_number', $winningNumber)
            ->first();
        ...
        if (!$existingResult) {
            ...
            DigitResult::create([
                'lottery_id' => $validatedData['lottery_id'],
                'digit_dashboard_id' => $dashboard->id,
                'winning_number' => (string) $winningNumber,
            ]);
        }

        // Fetch picked numbers for the current dashboard and lottery
        $pickedNumbers = DigitPickedNumber::where('lottery_id', $validatedData['lottery_id'])
        ...
        if ($pickedNumbers->isEmpty()) {
            ...
            DigitWinners::create([
                'lottery_id' => $validatedData['lottery_id'],
                'digit_lottery_dashboard_id' => $dashboard->id,
                'user_id' => 1,
                'winning_number' => $winningNumber,
                'price' => $winningPrice,
            ]);
            ...
            foreach ($pickedNumbers as $pickedNumber) {
                DigitWinners::create([
                    'lottery_id' => $pickedNumber->lottery_id,
                    'digit_lottery_dashboard_id' => $pickedNumber->digit_lottery_dashboard_id,
                    'user_id' => $pickedNumber->user_id,
                    'winning_number' => $pickedNumber->picked_number,
                    'price' => $winningPrice,
                ]);
